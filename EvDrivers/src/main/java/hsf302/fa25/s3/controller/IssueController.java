...
import hsf302.fa25.s3.dao.IssueDao;
...
public class IssueController {
    ...
    public ResponseEntity<?> create(@RequestBody CreateIssueRequest req) {
        if (req.getUserId() == null || req.getUserId().isBlank()
            ...
            return ResponseEntity.badRequest().body(Map.of("success", false, "message", "Thiếu dữ liệu bắt buộc"));
        }
        boolean ok = issueDao.insert(req.getUserId(), req.getStationId(), req.getDescription());
    ...
    public ResponseEntity<?> list(@RequestParam String role,
        ...
        if (!(isDriver || "Staff".equalsIgnoreCase(role) || "Admin".equalsIgnoreCase(role))) {
            return ResponseEntity.badRequest().body(Map.of("success", false, "message", "role không hợp lệ"));
        }
        if (isDriver && (userId == null || userId.isBlank())) {
            return ResponseEntity.badRequest().body(Map.of("success", false, "message", "EV Driver phải truyền userId"));
        }

        List<Issue> items = issueDao.listByVisibility(role, userId);
