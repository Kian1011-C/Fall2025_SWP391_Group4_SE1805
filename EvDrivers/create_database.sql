...
CREATE TABLE Swaps (
    ...
    status NVARCHAR(20) DEFAULT 'INITIATED' CHECK (status IN ('INITIATED','IN_PROGRESS','COMPLETED','FAILED','CANCELLED')),
    payment_id INT NULL,
...
SELECT COUNT(*) AS VehiclesCount FROM Vehicles;
SELECT COUNT(*) AS ServicePlansCount FROM ServicePlans;
SELECT COUNT(*) AS ContractsCount FROM Contracts;
SELECT COUNT(*) AS BatteriesCount FROM Batteries;
SELECT COUNT(*) AS SwapsCount FROM Swaps;
GO