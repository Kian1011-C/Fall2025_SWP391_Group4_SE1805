...
public class BatteryController {
    ...
    public ResponseEntity<?> getBatteryStatus(@PathVariable Long id) {
        try {
            Battery battery = batteryRepo.getBatteryById(id.intValue());
    ...
    public ResponseEntity<?> initiateBatterySwap(@RequestBody Map<String, Object> swapRequest) {
        ...
        return ResponseEntity.status(400).body(bad);
            ...
            try (java.sql.Connection conn = hsf302.fa25.s3.utils.ConnectDB.getConnection();
    ...
    public ResponseEntity<?> deleteBattery(@PathVariable Long id) {
        try {
            ...
            boolean deleted = batteryRepo.deleteBattery(id.intValue());
    ...
    public ResponseEntity<?> getBatteryStatistics() {
        try {
            Map<String, Integer> stats = batteryRepo.getBatteryStatistics();
        ...
        try {
            ...
            for (Map<String, Object> batteryData : batteryUpdates) {
                try {
                    ...
                    Battery existingBattery = batteryRepo.getBatteryById(id);
                    if (existingBattery == null) {
                        failCount++;
                    ...
                    if (batteryRepo.updateBattery(existingBattery)) {
                        successCount++;
                    } else {
                        failCount++;
