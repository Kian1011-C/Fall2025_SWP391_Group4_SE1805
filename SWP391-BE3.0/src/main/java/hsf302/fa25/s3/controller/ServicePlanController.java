...
public class ServicePlanController {
    ...
    public ResponseEntity<?> listActivePlans(HttpServletRequest req) {
        // Nếu cần chặn quyền admin cho trang quản trị thì mở check bên dưới:
    ...
    public ResponseEntity<?> addPlan(@RequestBody Map<String, Object> body) {
        ...
        try {
            ...
            String description = body.getOrDefault("description", null) == null ? null : String.valueOf(body.get("description"));
            ...
            if (error != null) return ResponseEntity.badRequest().body(Map.of("success", false, "message", error));

            // tên trùng?
            if (planDao.existsByName(planName)) {
                return ResponseEntity.badRequest().body(Map.of("success", false, "message", "Plan name already exists"));
            }
            ...
            if (newId <= 0) return ResponseEntity.status(500).body(Map.of("success", false, "message", "Create failed"));
            ...
            return ResponseEntity.ok(Map.of("success", true, "message", "Created", "data", data));
    ...
    public ResponseEntity<?> updatePlan(@RequestBody Map<String, Object> body) {
        ...
        try {
            ...
            if (planId == null) return ResponseEntity.badRequest().body(Map.of("success", false, "message", "planId is required"));

            ServicePlan existing = planDao.getPlanById(planId);
            if (existing == null) return ResponseEntity.status(404).body(Map.of("success", false, "message", "Plan not found"));

            String planName = body.get("planName") == null ? existing.getPlanName() : String.valueOf(body.get("planName"));
            BigDecimal basePrice = body.get("basePrice") == null ? existing.getBasePrice() : asBigDecimal(body.get("basePrice"));
            Integer baseDistance = body.get("baseDistance") == null ? existing.getBaseDistance() : asInt(body.get("baseDistance"));
            BigDecimal depositFee = body.get("depositFee") == null ? existing.getDepositFee() : asBigDecimal(body.get("depositFee"));
            String description = body.get("description") == null ? existing.getDescription() : String.valueOf(body.get("description"));
            boolean active = body.get("active") == null ? existing.isActive() : Boolean.parseBoolean(String.valueOf(body.get("active")));
            ...
            if (!ok) return ResponseEntity.status(500).body(Map.of("success", false, "message", "Update failed"));
            ...
            return ResponseEntity.ok(Map.of("success", true, "message", "Updated", "data", data));
    ...
    public ResponseEntity<?> deletePlan(@RequestBody Map<String, Object> body) {
        try {
            ...
            if (!ok) return ResponseEntity.status(500).body(Map.of("success", false, "message", "Delete failed"));

            return ResponseEntity.ok(Map.of("success", true, "message", "Deactivated"));
