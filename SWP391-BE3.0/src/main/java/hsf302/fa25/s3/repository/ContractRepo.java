...
public class ContractRepo {
        ...
        try (Connection conn = ConnectDB.getConnection();
            ...
            if (rs.next()) {
                ...
                if (baseDistance > 0 && monthlyDistance.compareTo(new java.math.BigDecimal(baseDistance)) > 0) {
                    ...
                    ServicePlanRepo servicePlanDao = new ServicePlanRepo();
    ...
    // 1) Xe có thuộc user không?
    public boolean vehicleBelongsToUser(int vehicleId, String userId) {
        String sql = "SELECT 1 FROM Vehicles WHERE vehicle_id = ? AND user_id = ?";
        try (Connection conn = ConnectDB.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, vehicleId);
    ...
    // 2) Lấy plan_id theo tên gói
    public Integer getPlanIdByName(String planName) {
        String sql = "SELECT plan_id FROM ServicePlans WHERE plan_name = ? AND is_active = 1";
        try (Connection conn = ConnectDB.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    // 3) Chặn overlap với hợp đồng đang active
    public boolean hasActiveOverlap(int vehicleId, java.sql.Date start, java.sql.Date end) {
        String sql = """
        SELECT 1
        FROM Contracts
        WHERE vehicle_id = ?
    ...
    """;
        try (Connection conn = ConnectDB.getConnection();
            ...
            ps.setDate(3, end);
            try (ResultSet rs = ps.executeQuery()) { return rs.next(); }
        } catch (Exception e) { e.printStackTrace(); return true; } // phòng thủ
    }

    // 4) Tạo hợp đồng: monthly_base_fee = base_price; monthly_total_fee = base_price
    public int createContract(int vehicleId, int planId,
                              java.sql.Date start, java.sql.Date end,
                              String signedPlace) {
        final String sqlGetPrice = "SELECT base_price FROM ServicePlans WHERE plan_id = ? AND is_active = 1";
        final String sqlInsert = """
        INSERT INTO Contracts
            (vehicle_id, plan_id, start_date, end_date, status,
             contract_number, signed_date, signed_place, current_month,
             monthly_distance, monthly_base_fee, monthly_overage_distance,
             monthly_overage_fee, monthly_total_fee, last_reset_date, created_at)
        VALUES (?, ?, ?, ?, 'active',
                ?, GETDATE(), ?, FORMAT(GETDATE(),'yyyy-MM'),
        ...
        try (Connection conn = ConnectDB.getConnection()) {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sqlGetPrice)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (!rs.next()) throw new RuntimeException("Plan không hợp lệ hoặc inactive: " + planId);
                    basePrice = rs.getDouble(1);
                }
            }
            // sinh mã CT-YYYY-xxxxxx
            String year = new java.text.SimpleDateFormat("yyyy").format(new java.util.Date());
            int rand = 100000 + new java.util.Random().nextInt(900000);
            String contractNo = "CT-" + year + "-" + rand;
            ...
            try (PreparedStatement ps = conn.prepareStatement(sqlInsert, java.sql.Statement.RETURN_GENERATED_KEYS)) {
                ...
                ps.setDate(3, start);
                ps.setDate(4, end);
                ps.setString(5, contractNo);
                ps.setString(6, (signedPlace == null || signedPlace.isBlank()) ? "Hà Nội" : signedPlace);
                ps.setDouble(7, basePrice); // monthly_base_fee
                ps.setDouble(8, basePrice); // monthly_total_fee
